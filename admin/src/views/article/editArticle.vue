...
export default {
    ...
    methods:{
        ...
        async createArticle(){
            ...
            const res = await this.$http.post('/crud/article',this.article)
            if(res.status == 200){
        ...
        async getArticleById(){
            const res =await this.$http.get(`/crud/article/${this.id}`)
            this.article = res.data
        },
        async putArticle(){
            const res = await this.$http.put(`/crud/article/${this.id}`,this.article)
            if(res.status == 200){
                this.$router.push('/articlesList')
                this.$message({
                    type: `success`,
